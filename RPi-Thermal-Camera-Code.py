...
def read_thermal_data():
    frame = np.zeros((24 * 32,), dtype=np.float32)  # MLX90640 resolution is 24x32
...
try:
    while True:
        ...
        # Normalize the thermal data to the defined temperature range
        thermal_data_normalized = np.clip((thermal_data - TEMP_MIN) / (TEMP_MAX - TEMP_MIN) * 255, 0, 255)

        # Convert the normalized thermal data to a color map for visualization
        thermal_data_colormap = cv2.applyColorMap(thermal_data_normalized.astype(np.uint8), cv2.COLORMAP_JET)

        # Stack the static colorbar next to the thermal image for better display
        stacked_image = np.hstack((thermal_data_colormap, colorbar))
